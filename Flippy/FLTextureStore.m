...
- (UIImage *)imageForKey:(NSString *)key
{
  ...
  if (!image) {
    [NSException raise:@"FLTextureStoreImageNotFound"
...
- (SKTexture *)setTextureWithImage:(UIImage *)image forKey:(NSString *)key filteringMode:(SKTextureFilteringMode)filteringMode
{
  ...
  if (!texture) {
    [NSException raise:@"FLTextureStoreTextureFailed" format:@"Could not create texture with passed image for key '%@'.", key];
  }
...
- (SKTexture *)setTextureWithImageNamed:(NSString *)imageName forKey:(NSString *)key filteringMode:(SKTextureFilteringMode)filteringMode
{
  ...
  if (!texture) {
    [NSException raise:@"FLTextureStoreTextureNotFound" format:@"Could not find texture with image name '%@' for key '%@'.", imageName, key];
  }
  texture.filteringMode = filteringMode;
  _textures[key] = texture;
...
- (SKTexture *)setTextureWithImageNamed:(NSString *)textureImageName andUIImageWithImageNamed:(NSString *)imageImageName forKey:(NSString *)key filteringMode:(SKTextureFilteringMode)filteringMode
{
  ...
  if (!texture) {
    [NSException raise:@"FLTextureStoreTextureNotFound" format:@"Could not find texture with image name '%@' for key '%@'.", textureImageName, key];
  }
  UIImage *image = [UIImage imageNamed:imageImageName];
  if (!image) {
    [NSException raise:@"FLTextureStoreImageNotFound" format:@"Could not find image with image name '%@' for key '%@'.", imageImageName, key];
  }
  texture.filteringMode = filteringMode;
  _textures[key] = texture;
  _images[key] = image;
