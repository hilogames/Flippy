...
static const CGFloat FLZPositionBackground = 0.0f;
static const CGFloat FLZPositionMenus = 1.0f;
...
{
  return (FLMenuItem *)[_items objectAtIndex:index];
}

@end

@implementation FLMenuScene
{
...
- (void)FL_createMenusNode
{
  _menusNode = [SKNode node];
  _menusNode.zPosition = FLZPositionMenus;
  [self addChild:_menusNode];
}
...
- (void)setBackgroundImageName:(NSString *)backgroundImageName
{
  ...
  if (backgroundImageName) {
    ...
    backgroundImageNode.zPosition = FLZPositionBackground;
    [self addChild:backgroundImageNode];
  }
}

- (void)FL_showMenu:(FLMenu *)menu
{
  if (_currentMenu) {
  ...
  for (NSUInteger i = 0; i < [menu itemCount]; ++i) {
    FLMenuItem *item = [menu itemAtIndex:i];
    HLLabelButtonNode *buttonPrototype = (item.buttonPrototype ? item.buttonPrototype : self.buttonPrototype);
    if (!buttonPrototype) {
      [NSException raise:@"FLMenuSceneMissingButtonPrototype" format:@"Missing button prototype for menu item."];
    }
